...
class FutureResolver:
    ...
    def save_resolve(
        self, message_type: str, topic: str, future: asyncio.Future, identifier: str | int | None
    ):
    ...
    def run_resolve_for_topic(self, message: dict[str, Any]):
    ...
    def merge_array_buffers(self, buffers: list[bytes]) -> bytes:
    ...
    def run_resolve_for_topic_for_file(self, message: dict[str, Any]):
        key = self.generate_message_key(
            message["type"],
            message.get("topic", ""),
            get_nested_field(message, "data", "uuid")
    ...
    def generate_message_key(
        self, message_type: str, topic: str, identifier: str | int | None
    ) -> str:
        return str(identifier) if identifier is not None else f"{message_type} $ {topic}"
